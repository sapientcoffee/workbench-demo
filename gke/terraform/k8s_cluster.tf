...
resource "kubernetes_secret" "db_iam_connection_info" {
  ...
  data = {
    host     = google_alloydb_instance.dev_instance.psc_instance_config[0].psc_dns_name
    username = google_alloydb_user.user1.user_id
    dbname   = google_alloydb_instance.dev_instance.instance_id
  }
}
